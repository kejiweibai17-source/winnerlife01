...
function getPageContent(locale: Locale) {
  return locale === "jp"
    ? jpMessages.equipmentSecurityPage
    : zhMessages.equipmentSecurityPage;
}
...
function getPagePath(locale: Locale) {
  return locale === "jp" ? "/jp/equipment/security" : "/equipment/security";
}
...
export function getEquipmentSecurityMetadata(locale: Locale): Metadata {
...
export function getEquipmentSecurityJsonLd(locale: Locale) {
  const page = getPageContent(locale);
  const seo = page.seo;
  ...
  const webPage = {
    ...
    workTranslation: {
      "@type": "WebPage",
      url: absoluteUrl(locale === "jp" ? "/equipment/security" : "/jp/equipment/security"),
